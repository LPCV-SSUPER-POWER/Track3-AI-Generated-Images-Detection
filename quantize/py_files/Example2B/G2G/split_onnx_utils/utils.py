...
def get_split_tensors(onnxfile, onnxmodel=None, include_first_input=True, include_all_adds=False):
    ...
    def get_chain_of_adds(nodes):
        ...
        '''
        def _dfs(add_node):
            if add_node.output[0] not in consumers:
                return [add_node.output[0]]
            longest = []
            for consumer in consumers[add_node.output[0]]:
                path = _dfs(consumer)
                if len(path) > len(longest):
                    longest = path
            return [add_node.output[0]] + longest

        adds = {node.output[0]: node for node in nodes if node.op_type in ('Add', 'Cast') }
        consumers = collections.defaultdict(list)
        for node_output, node in adds.items():
        ...
        for name, add in adds.items():
            ...
            if len(path) > len(max_path):
                max_path = path
        return [i for i in max_path if producers[i].op_type == 'Add']

    def can_produce(input_node, output_node):
        '''
        Check if input_node can be found in the producer chain
        '''
        if _seq(input_node) > _seq(output_node): return False
        stack, visited = collections.deque([output_node]), set()
        while stack:
            cur = stack.pop()
            if cur == input_node:
                return True
    ...
    def _producer(tensor):
        while producers[tensor].input[0] != 'inputs_embeds' and producers[tensor].op_type == 'Cast' :
            tensor = producers[tensor].input[0]
        return producers[tensor]

    def is_residual_add(output_tensor):
        x, y = [_producer(tensor) if tensor in producers else None for tensor in _producer(output_tensor).input]
        if x is None or y is None: return False

        # new_x = x + y
        x, y = (x, y) if _seq(x) < _seq(y) else (y, x)
        return can_produce(x, y)
    ...
    residual_add_outputs = [add_out for add_out in residual_add_outputs if is_residual_add(add_out)]
    ...
    if not include_all_adds:
        output_tensors += [output for idx, output in enumerate(residual_add_outputs) if idx%2==1]
