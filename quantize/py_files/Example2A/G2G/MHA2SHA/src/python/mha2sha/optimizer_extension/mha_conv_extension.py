...
class MhaConvExtension:
    ...
    def create_sha_conv_with_rope(self, 
        ...
        if self.mha2sha_optim.gqa_model:
            ...
            query_inp = self.mha2sha_optim._rope_extension.create_llama_rope_node(query_inp, cos_node, sin_node, head_dim, BranchType.Q)
            ...
            sin_node = info_dict["rope_sin_model_input"]
            # create_llama_rope_node will check if nchw_aligned then slice on c dim -> transpoe to NHWC accordingly.
            query_inp = self.mha2sha_optim._rope_extension.create_llama_rope_node(query_inp, cos_node, sin_node, head_dim, BranchType.Q) # [B, 1, L, D]
            key_inp = self.mha2sha_optim._rope_extension.create_llama_rope_node(key_inp, cos_node, sin_node, head_dim, BranchType.K) # [B, 1, L, D]
            # value_inp: [B, D, 1, L]
        ...
        # value_inp: [B, 1, L, D]
        qk_matmul_node, _ = self.mha2sha_optim._op_factory.get_matmul_op(query_inp, key_transpose, propose_sha_name+"_qk_MatMul")
    ...
    def create_sha_conv(self,
        ...
        if self.mha2sha_optim._scale_on_q_branch:
            query_inp = self.mha2sha_optim.add_scale_on_branch(info_dict, query_inp, propose_sha_name, sha_base_attn_node_list, BranchType.Q, field_name="scale")
        
        if self.mha2sha_optim._scale_on_k_branch:
            key_inp = self.mha2sha_optim.add_scale_on_branch(info_dict, key_inp, propose_sha_name, sha_base_attn_node_list, BranchType.K, field_name="k_scale")
        ...
        if self.mha2sha_optim.handle_past_key_value and self.mha2sha_optim.kv_cache:
            ...
            key_reshape_node, value_inp = (
                self.mha2sha_optim._past_kv_extension.concat_past_key_value_input(
                    past_key_inp, past_value_inp, key_reshape_node, value_inp, head_num
                )
            )
        ...
        qk_matmul_node, _ = self.mha2sha_optim._op_factory.get_matmul_op(query_transpose_node, key_reshape_node, propose_sha_name+"_qk_MatMul") # (B, H, W, H*W)
        sha_base_attn_node_list.qk_matmul.append(qk_matmul_node)
        self.mha2sha_optim.model.graph.node.append(qk_matmul_node)
