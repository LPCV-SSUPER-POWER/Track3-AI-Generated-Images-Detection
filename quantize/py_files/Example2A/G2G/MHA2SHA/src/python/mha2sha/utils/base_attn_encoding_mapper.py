...
class BaseAttnNode:
    ...
    k_scale:  Optional[Union[NodeProto, List[NodeProto]]] = None
...
def create_base_attn_mapping_dict(mha_base_attn_node)->Dict[str, NodeMappingDict]:
    ...
    return {
        ...
        "k_scale": create_activation_node_mapping_dict(
            input_1_name=None,
            input_2_name=mha_base_attn_node.k_scale.input[1] if mha_base_attn_node.k_scale is not None else None,
            output_name=mha_base_attn_node.k_scale.output[0] if mha_base_attn_node.k_scale is not None else None
        ),
