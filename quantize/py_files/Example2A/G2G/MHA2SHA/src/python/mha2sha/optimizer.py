...
from mha2sha.optimizer_extension.mha_concat_conv_extension import (
    MhaConcatConvExtension,
    update_concat_projections_encoding_name_to_rest_encoding_mapping_dict,
)
...
class MHA2SHAOptimizer:
    ...
    def __init__(
        ...
        self.concat_projections = concat_projections
        ...
        # LLama Models have shape [1, seq_len, vector_dim]
        if self.llm_model:
            self.seq_len = self.model.graph.input[0].type.tensor_type.shape.dim[1].dim_value
        ...
        self._mha_concat_conv_extension = MhaConcatConvExtension(self)
    ...
    def _run_sanity_checks(self):
        ...
        if self.concat_projections:
            assert self.mha_conv, "currently --concat-projections are only supported to be used with --mha-conv"
            assert not self.handle_past_key_value, "currently --concat-projections are only supported to be used without --handle-past-key-value"
            assert self.nchw_aligned, "currently --concat-projections are only supported to be used with --nchw-aligned"
            assert not self.lora_model , "currently --concat-projections are only supported to be used without --lora_model"
    ...
    def add_scale_on_branch(self, info_dict, input_node, propose_sha_name, sha_base_attn_node_list, branch_type, field_name="scale"):
        ...
        if branch_type == BranchType.Q:
            ...
            scale_op_type = self._scale_on_k_branch.op_type
            propose_sha_name = propose_sha_name + "_k"
        else:
            raise ValueError("unexpected branch_type")

        scale_value = info_dict[field_name]
        ...
        elif field_name == "k_scale":
            sha_base_attn_node_list.k_scale.append(scale_node)
        else:
            raise ValueError("unexpected field_name")
...
and expect when transposing C to dim=2 will have H in NCHW transposed to dim = 1 or 3. Can't determine head_num.")
    ...
    def get_dquery_dkey_dvalue(self, qk_matmul_node, qkv_matmul_node):
        ...
        if self.lora_model:
            ...
            if self.mha_conv and self.concat_projections:
                dquery, dkey, dvalue = self._mha_concat_conv_extension.get_qkv_info(qk_matmul_node, qkv_matmul_node)
            elif self.mha_conv:
        ...
        k_proj_end_node = dkey["concat_linear_node"].concat if "concat_linear_node" in dkey else dkey["matmul_node"]
        v_proj_end_node = dvalue["concat_linear_node"].concat if "concat_linear_node" in dvalue else dvalue["matmul_node"]
    ...
    def set_scale_from_const_input(self, d_qkv, const_node, field_name="scale"):
        ...
        d_qkv[field_name] = const_tensor_value  # no [0] because tensor is shaped correctly


    def set_scale_from_identity(self, d_qkv, identity_node, field_name="scale"):
        ...
        if scale_init.dims[0] == 1 and len(const_tensor_value) == 1:
            d_qkv[field_name] = const_tensor_value[0]  # only want scale not array
    ...
    def get_mha_pattern_info(self, d_qkv, qk_matmul_node, qkv_matmul_node):
        ...
        mha_base_attn_node = BaseAttnNode(
            ...
            k_scale = None, # in torch2, the exported onnx has both scale on q and k
                            # check https://github.com/pytorch/pytorch/blob/a8e7c98cb95ff97bb30a728c6b2a1ce6bff946eb/torch/onnx/symbolic_opset14.py#L175
            qk_scale = None,
            qk_softmax = None,
            qk_mask_add = None,
            qk_alibi_add = None,
        ...
        self._scale_on_k_branch = False # in torch2, the exported onnx has both scale on q and k
                                        # check https://github.com/pytorch/pytorch/blob/a8e7c98cb95ff97bb30a728c6b2a1ce6bff946eb/torch/onnx/symbolic_opset14.py#L175
        if ("Mul" not in node_list_pattern_set) and ("Div" not in node_list_pattern_set):
            ...
            if not self.llm_model:
                def find_scale_op(matmul_node):
                    try:
                        temp_scale_op = get_next_node_down_based_on_cond(matmul_node,
                            self.get_node_by_input_name,
                            node_found_cond=lambda n: n.op_type in ("Mul", "Div"),
                            node_end_search_cond=lambda n: n == qk_matmul_node
                        )
                        return temp_scale_op
                    except NodeNotFoundError as e:
                        log_debug(f"{e.__str__()}")
                    return None

                mha_base_attn_node.qk_scale = find_scale_op(d_qkv["query"]["matmul_node"])
                mha_base_attn_node.k_scale = find_scale_op(d_qkv["key"]["matmul_node"])


                def set_scale_info(temp_scale_op, field_name):
                    if temp_scale_op.input[1] in self.get_initializer_by_name.keys():
                        d_qkv[field_name] = numpy_helper.to_array(
                        ...
                        self.set_scale_from_identity(d_qkv, scale_identity, field_name=field_name)
                        if field_name in d_qkv:  # the above function only sets the field_name if it succeeds
                            return True
                    elif (scale_node := self.get_node_by_output_name[temp_scale_op.input[1]]).op_type == "Constant":
                        self.set_scale_from_const_input(d_qkv, scale_node, field_name=field_name)
                        return True
                    return False

                if set_scale_info(mha_base_attn_node.qk_scale, "scale"):
                    self._scale_on_q_branch = mha_base_attn_node.qk_scale

                if set_scale_info(mha_base_attn_node.k_scale, "k_scale"):
                    self._scale_on_k_branch = mha_base_attn_node.k_scale
    ...
    def get_attention_subgraph_info(self, start_node_name: str, end_node_name: str) -> Dict:
        ...
        elif self.concat_projections:
            q_proj_end_node = d_qkv["query"]["concat_linear_node"].concat
        else:
        ...
        reshape_node = get_next_node_down_based_on_cond(q_proj_end_node,
                                                        ...
                                                        node_end_search_cond=lambda n: n == qk_matmul_node)
        matmul_input_shape = get_node_input_constant_op_value(reshape_node, self.get_node_by_output_name)
    ...
    def get_qkv_input(self, info_dict):
        # qkv_input are the input nodes for qkv matmul
        qkv_input = []
    ...
    def create_sha_pattern(self, info_dict, ns, head_num, value_inp, sha_nodes_list, sha_base_attn_node_list):
        ...
        qkv_matmul, _ = self._op_factory.get_matmul_op(softmax_node, value_inp, propose_sha_name+"_qkv_MatMul")
    ...
    def create_sha(self,
        ...
        if self.handle_rope_ops:
            if self.use_position_embedding:
                ...
                query_inp = self._rope_extension.create_llama_rope_node(query_inp, cos_node, sin_node, head_dim, BranchType.Q)
                key_inp = self._rope_extension.create_llama_rope_node(key_inp, cos_node, sin_node, head_dim, BranchType.K)
            else:
        ...
        if self.handle_past_key_value and self.kv_cache:
            ...
            key_transpose, value_inp = self._past_kv_extension.concat_past_key_value_input(past_key_inp, past_value_inp, key_transpose, value_inp, head_num)
        ...
        if self._scale_on_q_branch:
            query_inp = self.add_scale_on_branch(info_dict, query_inp, propose_sha_name, sha_base_attn_node_list, BranchType.Q, field_name="scale")

        # Step 5 QK matmul
        qk_matmul_node, _ = self._op_factory.get_matmul_op(query_inp, key_transpose, propose_sha_name+"_qk_MatMul")
    ...
    def create_sha_func_handler(self, query_lora_inp, key_lora_inp, value_lora_inp):
        ...
        if self.mha_conv and self.handle_rope_ops and self.nchw_aligned:
            ...
            if self.concat_projections:
                create_sha_func = self._mha_concat_conv_extension.create_sha_conv
            else:
    ...
    def optimize(self) -> ModelProto:
        ...
        for ns, (start_node_name, end_node_name) in enumerate(start_end_node_names):
            ...
            assert block_size[0] == block_size[1] == block_size[2], "head_dim should be the same for q, k, and v projection"
            ...
            self._mha_concat_conv_extension.reset_sha_encoding_name_list()
            ...
            for head_num in range(info_dict["num_heads"]):
                ...
                create_sha_func, extension_kwargs = self.create_sha_func_handler(query_lora_inp, key_lora_inp, value_lora_inp)
                qkv_matmul, key_inp, value_inp, past_value_inp = create_sha_func(info_dict,
                                                                                 ns,
                                                                                 head_num,
                                                                                 head_dim,
                                                                                 query_matmul_inp,
            ...
            if self.concat_projections:
                update_concat_projections_encoding_name_to_rest_encoding_mapping_dict(
                    encoding_mapping_dict.concat_conv,
                    self._mha_concat_conv_extension.sha_nodes,
                    info_dict
                )
