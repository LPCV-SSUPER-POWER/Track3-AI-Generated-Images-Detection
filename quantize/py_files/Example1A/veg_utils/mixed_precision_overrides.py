...
def apply_input_output_exception(quantizer, exception):
    ...
    data_type = exception.get("data_type", "int")
    if quantizer is None or quantizer.data_type!=data_type:
        quantizer = LazyQuantizer(exception.get("bitwidth", 16),
                                  'nearest',
                                  QuantScheme.post_training_tf,
                                  exception.get("asymmetric", True) is False,
                                  enabled_by_default=True,
                                  data_type=QuantizationDataType.float if exception.get("data_type", "int") == "float" else QuantizationDataType.int).get_v1_quantizer()
...
def apply_param_exception(quantizer, exception):
    ...
    data_type = exception.get("data_type", "int")
    if quantizer.data_type!=data_type:
        quantizer = LazyQuantizer(exception.get("bitwidth", 16),
                                  'nearest',
                                  QuantScheme.post_training_tf,
                                  exception.get("asymmetric", True) is False,
                                  enabled_by_default=True,
                                  data_type=QuantizationDataType.float if data_type == "float" else QuantizationDataType.int).get_v1_quantizer()
    else:
        if exception.get("enabled", True) is False:
            quantizer._allow_overwrite = False
...
class ManualQuantsimMixedPrecisionConfig:
    ...
    def apply_param_exception_to_module(self, param_exceptions, module):
        ...
        if module.param_quantizers.get('weight',None) is None:
            return
